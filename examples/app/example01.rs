...
fn main() {
    App::build()
        ...
        .add_update_system(keyboard_input_system(), "default")
        // .add_update_system(empty_system(), "default")
        .finish()
        .run();
}
...
#[system]
fn keyboard_input(#[resource] input: &Input) {
    let mut btns = [(KeyCode::F, -1), (KeyCode::Space, -1), (KeyCode::T, -1)];

    for (btn, state) in btns.iter_mut() {
        if input.keyboard.just_pressed(*btn) {
            *state = 0;

            println!("--------just pressed: {:?}", btn);
        }

        if input.keyboard.just_released(*btn) {
            *state = 1;

            println!("--------just released: {:?}", btn);
        }

        if input.keyboard.pressed(*btn) {
            *state = 2;
        }

        if input.keyboard.released(*btn) {
            *state = 3;
        }
    }

    println!("F: {}, Space: {}, T: {}", btns[0].1, btns[1].1, btns[2].1);
}
